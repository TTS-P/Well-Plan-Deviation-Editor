...
from wellplan.interpolate import interpolate_survey, regrid_survey
...
with st.sidebar:
    ...
    # Spline type is locked to cubic (smoothest) — not user-selectable.
    kind = "cubic"
...
# Output is always re-gridded onto uniform MD stations at the resample interval
# (set in the sidebar). This is locked on — not user-selectable.
out_base = regrid_survey(modified, step=step, kind=kind)

display = out_base  # the survey shown / exported (smooth unless tortuosity is on)

with st.sidebar:
    st.header("Tortuosity")
